...
import opts #解析命令行参数
import models
import eval_utils_tsg #包含了用于评估图像描述生成模型的实用函数
import misc.utils as utils
from dataloader_tsg import *  #用于加载训练数据
from misc.rewards import init_scorer  #用于生成奖励信号。
from misc.loss_wrapper_tsg import LossWrapper  #用于封装损失函数。
...
#将指定的标量值添加到 Tensorboard 中，以便在 Tensorboard 的可视化界面中进行监视和分析。
#在调用此函数之前，需要在代码中创建一个 TensorboardX 的 SummaryWriter 对象，并将其传递给 writer 参数。
def train(opt):
    ...
    if opt.start_from is not None:
        ...
        with open(
                os.path.join(opt.checkpoint_path, 'infos_' + opt.id + format(int(opt.start_from), '04') + '.pkl'),
                'rb') as f:
    ...
    lw_model = LossWrapper(model, opt) #使用 LossWrapper 类将模型包装起来，用于训练时的损失函数计算
